...
export const compileCommandResult = async (
  ...
  if (eventsResult.isErr()) {
    return err(toCommandError("compile", formatRecordStoreError(eventsResult.error)));
  }
  ...
  if (await exists(p)) {
    ...
    if (prevResult.isErr()) {
      return err(toCommandError("compile", formatRecipeStoreError(prevResult.error)));
    }
  ...
  if (saveResult.isErr()) {
    return err(toCommandError("compile", formatRecipeStoreError(saveResult.error)));
  }
  return ok(undefined);
};
